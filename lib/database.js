...
// For example:
// try {
//   db.prepare("ALTER TABLE accounts ADD COLUMN new_col TEXT").run();
// } catch (e) {
...
export function createOrUpdateConversation(convKey, apiKeyHash, qwenChatId, accountId, messageCount, lastMsgId) {
  const insertOrReplace = db.prepare(`
    INSERT INTO conversations (conv_key, api_key_hash, qwen_chat_id, account_id, message_count, last_msg_id, last_used)
  ...
  `);
  insertOrReplace.run(convKey, apiKeyHash, qwenChatId, accountId, messageCount, lastMsgId);
}
